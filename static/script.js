...
function addMessage(room, username, message, push = false) {
    if (push) STATE[room].push({ username, message })

    if (STATE.room == room) {
        var node = messageTemplate.content.cloneNode(true);
        node.querySelector(".message .username").textContent = username;
        node.querySelector(".message .username").style.color = hashColour(username);
...
function subscribe(uri) {
    ...
    function connect(uri) {
        ...
        events.addEventListener("open", () => {
            setConnnectedStatus(true);
            console.log(`connected to event stream at ${uri}`);
            retryTime = 1;
        });

        events.addEventListener("error", () => {
            setConnnectedStatus(false);
            events.close();
...
function setConnnectedStatus(status) {
...
function init() {
    ...
    addMessage("lobby", "SERVER", "This is the lobby", true);
    ...
    newRoomForm.addEventListener("submit", (e) => {
        ...
        addMessage(room, "SERVER", `You joined the "${room}" room.`, true);
